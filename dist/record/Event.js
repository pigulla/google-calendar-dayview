...
class Event extends Record({
    ...
    static async from_json(event, user_cache) {
        ...
        const participant_emails = event.attendees.filter(attendee => !attendee.resource).filter(attendee => attendee.responseStatus === 'accepted').map(attendee => attendee.email);
        const confirmed = event.attendees.filter(attendee => attendee.self && attendee.responseStatus === 'accepted').length > 0;

        const [participants, creator, organizer] = await Promise.join(Promise.map(participant_emails, email => user_cache(email, event.summary)), user_cache(event.creator.email, event.summary), user_cache(event.organizer.email, event.summary));
